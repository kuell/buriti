...
class InvestigacaoView extends RelatorioController {
	...
	function pagina3($investigacao){
		...
		for ($i = 1; $i < 5; $i++) {
			$this->Cell( 70, 8, utf8_decode(''), 1, 0, 'L');
			$this->Cell( 40, 8, utf8_decode(' '), 1, 0, 'L');
			$this->Cell( 40, 8, utf8_decode(''), 1, 0, 'L');
			$this->Cell( 40, 8, utf8_decode(''), 1, 0, 'L');
			$this->Ln();
		}
		...
		$this->Cell( 0, 6, utf8_decode('Comentários: '), 'RL', 0, 'L');
		$this->Ln();
		
		$this->Cell( 0, 10, utf8_decode(' '), 'RL', 0, 'L');
		$this->Ln();
	...
	function capa($investigacao){
		...
		$this->Cell(0, 8, utf8_decode('Ficha de investigação emitida em: '.date('d/m/Y H:i:s')), 'RL', 0, 'L', 0);
		$this->Ln();
...
(     )  FATOR OPERACIONAL  > ENVOLVE APECTOS FISICOS RELACIONADOS Á ORGANIZAÇÃO DA PRODUÇÃO E AS CONDIÇÕES DO MEIO AMBIENTE DE TRABALHO , PROTEÇÃO DE MAQUINAS E EQUIPAMENTOS , COMO POR  EXEMPLO : FERRAMENTAS IMPROVISADAS , PROTEÇÕES INEXISTENTES E DESEQUILIBRIO NO CICLO DE TRABALHO-DESCANSO , ENTRE OUTROS;'), 'LRB', 'L');
...
COMPORTAMENTAIS; ESTÁ RELACIONADA Á PREDISPOSIÇÃO E AS MOVITAÇÕES DO INDIVIDUO NA EXECUÇÃO DA ATIVIDADE, COMO, POR EXEMPLO: NA OPERAÇÃO DE EQUIPAMENTOS SEM AUTORIZAÇÃO, NÃO SEGUIR E/ OU PULAR ETAPAS DO PROCEDIMENTO CILMA SOCIAL NO TRABALHO AÇÕES IMPREVISÍVEIS ETC;
CARACTERISTICAS DO MEIO AMBIENTE DE TRABALHO: ENVOLVE ASPECTOS FISICOS, COMO POR EXEMPLO; AMBIENTE RUIDOSO, ILIMUNINAÇÃO PRECARIA, TEMPERATURAS EXTREMAS, TRABALHO NORTUNO, CONFINADO EMBARCADO, FALTA HIGIENE E ORGANIZAÇÃO, CONDIÇOES METEOROLÓGICAS ETC;
TIPO DE ATIVIDADES; RELACIONADAS AOS RISCOS INERENTES DA ATIVIDADE.'), 'LRB', 'L');
	}
 ...
 (   ) CONDIÇÕES ABAIXO DO PADRÃO, (CONDIÇÃO INSEGURA ); PROTEÇÃO AUSENTE  OU INADEQUADA , PERIGO DE EXPLOSÃO OU INCENDIO , AUSENCIA  DE ORDEM E LIMPEZA , EXPOSIÇÃO AOS AGENTES AMBIENTAIS NOCIVOS .'), 'RL', 'L', 0);
